...
public class SenderNIO extends ExperimentEntryPoint {

    public static void main(String[] args) throws Exception {
        parseArgs("SenderNIO", args);
        ...
        if(isrdma){
            if(isnaos){
                ...
                rdmachannel = new ch.ethz.rdma.RdmaClient(host, port).lazy_connect();
            }
        } else {
            if(isnaos){
        ...
        // running test...
        if(isnaos) {
            if(batch_size == 0) { // no iterator
                if(naosarray){
                    if (isrdma) {
                        BenchmarkNaos.runLatencyTestSenderArray(rdmaep, obj, serializerImpl, iters, nodes);
                    } else {
                        BenchmarkNaos.runLatencyTestSenderArray(socket, obj, serializerImpl, iters, nodes);
                    } 
                } else {
                    if (isrdma) {
                        BenchmarkNaos.runLatencyTestSender(rdmaep, obj, serializerImpl, iters);
                    } else {
                        BenchmarkNaos.runLatencyTestSender(socket, obj, serializerImpl, iters);
                    } 
                }
            } else { // use iterator
                if (isrdma) {
                    BenchmarkNaos.runLatencyTestSender(rdmaep, it, serializerImpl, iters, batch_size);
                } else {
                    BenchmarkNaos.runLatencyTestSender(socket, it, serializerImpl, iters, batch_size);
                }
            }
        } else { // not naos
            if (isrdma) {
                BenchmarkRDMA.runLatencyTestSender(rdmachannel, obj, serializerImpl, iters);
            } else {
                BenchmarkTCP.runLatencyTestSender(channel, obj, isdirect, serializerImpl, iters);
            }
        }
