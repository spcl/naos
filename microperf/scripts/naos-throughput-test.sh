...
# 
# Simple script to run a throughput test
#
...
Script for starting Naos throughput test.
usage  : $0 [options]
options: --server=HOST,DEVICEIP:PORT   # IP address of the server
         ...
         --offheap         # use offheap memory on the tests. For naos this flag enables async send.
         --size=INT # the size of experiment e.g., the number of elements in an array
         --time=INT # the duration of test in ms
         --tput=INT # target thoutghput in req/sec. default unlimited
...
main_recver="ch.ethz.microperf.ReceiverThroughput"
main_sender="ch.ethz.microperf.SenderThroughput"
...
do
    ...
    --batch=*)
        ...
        ;;
    --time=*)
        time=`echo $arg | sed -e 's/--time=//'`
        time=`eval echo ${time}`    # tilde and variable expansion
        ;;
    --tput=*)
        tput=`echo $arg | sed -e 's/--tput=//'`
        tput=`eval echo ${tput}`    # tilde and variable expansion
        ;;
    --offheap)
        offheap="--offheap"
        ;;
...
if [ -z "$output" ]; then
  output="throughput_${network}_${serializer}_${data}_${size}_${time}_${tput}"
  ...
  if [ ! -z "${offheap}" ]; then 
      if [ ${serializer} == "naos" ]; then
        output="${output}_async"
      else
...
jvmparams=" ${libpath} -XX:+UnlockDiagnosticVMOptions -XX:+UnlockExperimentalVMOptions \
 -XX:+UseShenandoahGC -XX:ShenandoahGCMode=passive -XX:-UseBiasedLocking -Xms3g \
-Xmx3g -XX:+AlwaysPreTouch -XX:ShenandoahHeapRegionSize=32m -XX:+UseCompressedOops ${naosnocycles}"
codeparams="-d ${data} -e ${size} -h ${serverdev}  -m ${time} -t ${tput} -n ${network} -p ${serverport} -s ${serializer} -b ${iterative_batch} ${offheap} ${array}"

recvcommand="${PRELOAD} timeout -s 9 16s ${JAVADIR}/java $jvmparams -cp ${WORKDIR}/${jar} -ea  $main_recver $codeparams"
sendcommand="${PRELOAD} timeout -s 9 16s ${JAVADIR}/java $jvmparams -cp ${WORKDIR}/${jar} -ea  $main_sender $codeparams"
