...
#include <assert.h>
#include "c-rbtree-stdaux.hpp"
#include <stdalign.h>
#include <stddef.h>
#include "c-rbtree.hpp"
#include "c-rbtree-private.hpp"
 ...
 */
static inline void c_rbnode_swap_child(CRBNode *o, CRBNode *n) {
        CRBNode *p = c_rbnode_parent(o);

        if (p) {
                if (p->left == o)
                        c_rbtree_store(&p->left, n);
                else
                        c_rbtree_store(&p->right, n);
        }
}
...
static inline CRBNode *c_rbtree_paint_path(CRBNode *n) {
        ...
        for (;;) {
                ...
                if (!p) {
                         ...
                         */
                        c_rbnode_set_parent_and_flags(n, (CRBNode *)c_rbnode_raw(n), c_rbnode_flags(n) & ~C_RBNODE_RED);
                        return NULL;
                ...
                c_rbnode_set_parent_and_flags(g, (CRBNode *)c_rbnode_raw(g), c_rbnode_flags(g) | C_RBNODE_RED);
                n = g;
        }
}
